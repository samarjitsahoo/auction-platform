...
export const republishItem = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!req.body.startTime || !req.body.endTime) {
        return next(new ErrorHandler('StartTime and EndTime for republihsh is mandatory.', 400));
    }
    if(new Date(auctionItem.endTime) > Date.now()) {
        return next(new ErrorHandler('Auction is still active. Can not republish', 400));
    }
    let data ={
    ...
    if(data.startTime>= data.endTime) {
        return next(new ErrorHandler('Starting time must be less than ending time.', 400));
    }
